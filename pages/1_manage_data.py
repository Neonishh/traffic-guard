...
with tab1:
    ...
    with st.expander("🗑️ Delete Driver"):
        if drivers:
            ...
            if st.button("Delete") and st.checkbox("Confirm"):
                query = "DELETE FROM Driver WHERE Driver_ID=%s"
                success, msg = db.execute_query(query, (opts[to_del],))
                if success:
                    st.success("✅ Deleted!")
                    st.rerun()
                else:
...
# ==================== VEHICLES (FIXED - DROPDOWN FOR DRIVER) ====================
with tab2:
    st.header("Vehicles")
    ...
    # Add vehicle - DROPDOWN FOR DRIVER!
    ...
    with st.expander("🗑️ Delete Vehicle"):
        if vehicles:
            ...
            if st.button("Delete", key="del_veh") and st.checkbox("Confirm", key="conf_v"):
                query = "DELETE FROM Vehicle WHERE Vehicle_ID=%s"
                success, msg = db.execute_query(query, (vopts[to_del],))
                if success:
                    st.success("✅ Deleted!")
                    st.rerun()
                else:
...
with tab4:
    ...
    with st.expander("➕ Record Violation"):
        st.info("💡 Penalty auto-created by trigger!")
            ...
            with st.form("add_violation"):
                ...
                if st.form_submit_button("Record Violation"):
                    if not location:
                        ...
                        # NO manual IDs - all from dropdowns!
                        ...
                        if success:
                            ...
                            st.info("🔔 Penalty auto-created!")
                            st.rerun()
                        else:
                            st.error(f"❌ {msg}")

# ==================== PAYMENTS (FIXED - DROPDOWN FOR PENALTY) ====================
with tab5:
    st.header("Payments")
...
# ==================== APPEALS (FIXED - DROPDOWN FOR VIOLATION) ====================
with tab6:
    st.header("Appeals")
